...
S1 (ExposureShare@K Drift) similarly gained a 95% bootstrap confidence interval on the drift statistic itself (`es_at_k_drift_ci()`), classified via a dedicated `classify_s1_ci()` rather than the shared `classify_ci()` (the boundary conventions differ at exactly the -10%/-20% threshold values). This closed a 92.5% floor-scale (~42 events/week) family-wise false-alert rate — the same missing-uncertainty-quantification defect as above, never previously ported to S1. At that same floor-scale volume the fix has a real, quantified cost — detection power on a large real regression drops from ~100% to roughly 60–80%, and the standard's "sustained > 2 weeks" escalation path becomes effectively unreachable — which is explicitly flagged as requiring Volume 4 governance sign-off (§A5/§A7), not resolved by the implementation itself; see `docs/Validation_Addendum_S1_Confidence_Intervals.md`.
